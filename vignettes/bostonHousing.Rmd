...
actuals <- original$ptratio[is.na(BostonHousing$ptratio)]  # take the original values
...
```{r}
BostonHousing <- original.NA
se.0 <- sd(BostonHousing$ptratio, na.rm = TRUE) / sqrt(sum(!is.na(BostonHousing$ptratio)))
se.0
```
...
se.1 <- sd(BostonHousing$ptratio) / sqrt(sum(!is.na(BostonHousing$ptratio)))
se.1
```
