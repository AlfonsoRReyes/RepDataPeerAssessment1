...
```{r}
# Before
df <- data.frame(activity$steps)

my.summary <- function(x,...){c(
  mean   = mean(x, ...),
  sd     = sd(x, ...),
  median = median(x, ...),
  min    = min(x, ...),
...
info.imp <- function(x, uniVar, colName, decs = 3) {
  if (anyNA(x[, uniVar])) {
    ...
    indNA.not <- which(!((1:nrow(x)) %in% indNA))  # 
...
info.imp(imp.df, indVar, "After", 2)

# get information before imputing data
info.imp(activity, indVar, "Before", 2)
```
