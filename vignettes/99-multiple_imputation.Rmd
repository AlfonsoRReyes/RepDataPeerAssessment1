...
imp <- actbycol.imp.melt                    # the imputted data frame
...
plot(density(imp[indNA, indVar]), 
     main = "Observed and imputed values", 
     xlab = "steps")
lines(density(imp[indNA.not, indVar]), 
      col = "red")
legend("topright",text.col = c("black","red"),legend = c(" Imputation","Observed values"))
...
### Plot `x` vs `y` of random imputting
```{r fig.asp=1}
...
points(x.norm[indNA], y.norm[indNA], col = "red")
```
