...
# steps.imp <- imp$imp[indVar][[1]]   # all imputed values ONLY; no observations
# 
# imp.df[indNA, indVar] <- steps.imp

imp.df <- get.imp.df(activity, imp, "steps")

# get information before imputing data
df.before <- info.imp(activity, indVar, "Before", 2)

# get information after imputing data
df.mean <- info.imp(imp.df, indVar, "Mean", 2)

cbind(df.before, df.mean)
...
### Build a plot of normalized values
