...
#### Saving the raw dataset to `data` folder
...
**Convert the variable `date` as.factor to `as.date`**
```{r}
...
The function used is na.interp from the package 'forecast'. 
...
```{r}
activity.imp$weekdays <- weekdays(activity.imp$date)
unique(activity.imp)
```
...
```{r}
unique(weekdays(activity.imp$date))
```

```{r}
res <- ifelse(weekdays(activity.imp$date) %in% c("Saturday", "Sunday"), "weekend", "weekday")
